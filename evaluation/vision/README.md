...
如果暂时不做人工标注，可以先检查识别接口的结构安全边界，不评价颜色或品类
是否准确：

```powershell
python -m scripts.check_wardrobe_vision_contract
```

该检查覆盖单件/多件结果、唯一 `draft_id`、置信度范围、必需的用户确认标记、
缺失字段声明和单张图片结果数量上限。它不会调用外部视觉模型，也不会读取真实
用户图片。
