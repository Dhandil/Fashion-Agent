...
模型输入由统一 Context Package 管理。当前事实优先级最高；明确记忆、历史记忆
和 RAG 知识分别使用独立字符预算，总字符预算作为最终外层上限，避免某一类内容
挤占全部窗口。系统使用本地确定性规则提供近似 Token 指标，用于容量观测而不是
供应商精确计费或模型窗口硬限制。日志只记录条目数、字符数、估算 Token 和截断
数量，不记录用户正文、提示词或检索内容。
Context Package 还会把来源标识、路径、版本和更新时间作为独立元数据保存；
相同正文只注入一次，但合并全部命中来源，同一来源标识出现矛盾版本时仅记录
冲突计数和候选键，不把正文写入日志。
Graph、LLM、RAG、Tool 和外部 Provider 共用零依赖操作计时边界，成功事件记录
耗时与非敏感数量，失败事件增加异常类型后继续交给原业务降级或异常处理逻辑。
用户和会话只以进程级随机 HMAC 短标识进入日志，既能关联同一进程内的请求，
又不会持久化原始身份标识或形成跨重启用户追踪。
...
- `POST /api/v1/style-profile/candidates/confirm`：重新校验当前证据后，把
  用户确认的候选写入档案。
- `GET /api/v1/style-profile/memories`：查询当前用户已确认偏好的来源、证据、
  首次确认时间、最近确认时间和可选过期时间；默认不返回已过期记录。
- `PATCH /api/v1/style-profile/memories/{preference_memory_id}`：设置过期时间，
  显式传 `null` 可恢复为长期有效；记录不存在或时间冲突时返回统一错误。
- `DELETE /api/v1/style-profile/memories/{preference_memory_id}`：幂等删除单条
  审计记录，并同步移除它对 Style Profile 产生的同向偏好影响。
- `DELETE /api/v1/style-profile`：删除当前用户的长期穿搭档案；重复删除保持
  幂等，不通过响应差异泄露档案是否曾经存在。
...
反馈候选带有来源 `outfit_feedback` 和稳定 `candidate_id`。该 ID 由候选类别、
方向、规范化值、支持证据和反向证据的 Outfit ID 集合计算。确认接口必须带回
同一个 ID；证据被删除、增加、改变方向或低于阈值后，旧 ID 自动失效。

用户确认候选后，Style Profile 和 `preference_memories` 审计记录使用同一个
请求级数据库事务写入。审计记录按照“用户 + 类别 + 规范化值”唯一，重复确认
保留首次确认时间并更新最近确认时间、方向和证据。记录只保存稳定 Outfit ID，
不复制 Outfit 正文、反馈正文、聊天内容、Prompt、天气或 RAG 片段。
同一偏好的证据方向翻转后，确认操作会更新原记录并从 Style Profile 的反向
列表移除该值，不创建相互冲突的喜欢/避免副本。

过期时间是可选治理字段；当前确认流程默认不设置自动过期。以后启用期限时，
过期记录默认不会出现在查询结果或 Agent 上下文中，用户再次明确确认可以重新
激活。用户可以单独删除一条偏好或删除完整 Style Profile；两种操作都在请求级
事务中同步维护档案与审计记录。
